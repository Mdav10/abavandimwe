...
# ========== HTML (YOUR ORIGINAL FULL HTML, with voice/image additions) ==========
HTML = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function addMessage(sender, text, isSent, timestamp, messageId, replyTo, voiceUrl, mediaUrl, mediaType) {
    ...
    // Add reaction picker and reply actions
    let actionHtml = `
        <div class="message-actions" style="display:flex; gap:6px; margin-top:4px; flex-wrap:wrap;">
            ...
            <button onclick="toggleReactionPicker(this)" style="background:transparent;border:none;color:#888;font-size:11px;cursor:pointer;">😊</button>
            <div class="reaction-picker" style="display:none;position:absolute;bottom:100%;left:0;background:#050508;border:1px solid #0f0;border-radius:8px;padding:4px;z-index:100;flex-wrap:wrap;gap:2px;max-width:160px;">
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                await sendMessageWithMedia(text, data.url, data.type || file.type);
            } else {
...
async function toggleReaction(messageId, emoji) {
    ...
    try {
        ...
        const data = await response.json();
        if (data.success) {
            // Update local reactions display if needed
            // We don't store reactions in memory, just rely on server broadcast
        }
    } catch (error) {
