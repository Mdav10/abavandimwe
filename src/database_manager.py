...
class Database:
    ...
    async def get_messages(self, group: str, limit: int = 100) -> List[Dict]:
        def _get():
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
            c.execute('SELECT ciphertext, sender, salt, created_at FROM messages WHERE group_name = ? AND created_at > datetime("now", "-24 hours") ORDER BY created_at ASC LIMIT ?', (group, limit))
            rows = c.fetchall()
            conn.close()
